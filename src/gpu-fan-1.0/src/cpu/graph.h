...
class Graph {
  ...
  int shortestPathBetweennessUnweighted(vector<float>& res);
  int shortestPathBetweennessWeighted(vector<float>& res);
...
//int readGraph(string ppi_file, string file_name, fileType ft, weightType wt);
  ...
  int shortestPathBetweenness(vector<float>& res);
  int closeness(vector<float>& res);
