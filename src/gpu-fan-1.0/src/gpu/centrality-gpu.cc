...
int main(int argc, char** argv){
  ...
  while ((c=getopt(argc, argv, "bcesg:o:d")) != -1 ) {
    switch(c){
      case 'b':
        bc_flag=true;
        break;
  ...
  int *host_v, *host_e;
  host_v=(int *)malloc(sizeof(int)*n_count);
  if(!g.directed()){
    e_count*=2;
  }
  host_v=(int *)malloc(sizeof(int)*e_count);
  host_e=(int *)malloc(sizeof(int)*e_count);
  /* no matter what the type of edge weight is (int, float),
   * the betweenness will be a float 
   */
  create_adjacency_arrays(g, host_v, host_e);

  if(bc_flag){
    float *host_bc;
    host_bc=(float *)malloc(sizeof(float)*n_count);
...
#endif
    bc(g.directed(), n_count,e_count,host_v,host_e, host_bc);
#ifdef _PROFILE
  ...
  if(cc_flag || ec_flag){
    float *host_cc;
    host_cc=(float *)malloc(sizeof(int)*n_count);
    if(cc_flag){
      for(int i=0; i<n_count; i++){
        host_cc[i]=0;
      }
      ...
      gettimeofday (&start, NULL);
#endif
      cc(n_count,e_count,host_v,host_e, host_cc,false);
#ifdef _PROFILE
      gettimeofday (&finish, NULL);
#endif
      map<string,float> cc_map;
      for(int i=0; i<n_count; i++){
        cc_map[g.id2node(i)]=host_cc[i];
      }
    ...
    if(ec_flag){
      ...
      gettimeofday (&start, NULL);
#endif
      cc(n_count,e_count,host_v,host_e, host_cc,true);
#ifdef _PROFILE
      gettimeofday (&finish, NULL);
#endif
      map<string,float> cc_map;
  ...
  if(sc_flag){
    int *host_sc;
    host_sc=(int *)malloc(sizeof(int)*n_count);
    for(int i=0; i<n_count; i++){
      host_sc[i]=0;
    }
#ifdef _PROFILE
    gettimeofday (&start, NULL);
#endif
    sc(g.directed(), n_count,e_count,host_v,host_e, host_sc);
#ifdef _PROFILE
    gettimeofday (&finish, NULL);
#endif
    map<string,int> sc_map;
    for(int i=0; i<n_count; i++){
      sc_map[g.id2node(i)]=host_sc[i];
    }
